...
# ---- LẤY THAM SỐ TỪ ARGS/ENV ----
if len(sys.argv) >= 3:
    input_csv = sys.argv[1]
    output_parquet = sys.argv[2]
else:
    input_csv = os.getenv("INPUT_CSV")
    output_parquet = os.getenv("OUTPUT_PARQUET")

if not input_csv or not output_parquet:
    raise ValueError("Missing INPUT_CSV or OUTPUT_PARQUET")

# ---- ĐỌC CSV (đặt trước mọi transform) ----
df = spark.read \
    .option("header", True) \
    .option("quote", '"') \
    .option("escape", '"') \
...
# # --- LẤY THAM SỐ TỪ KESTRA ---
# # Ưu tiên argv (đã truyền trong commands), fallback sang env để linh hoạt
# if len(sys.argv) >= 3:
#     input_csv = sys.argv[1]
#     output_parquet = sys.argv[2]
# else:
...
# # Ghi đúng 1 file parquet
# tmp_dir = output_parquet + ".tmpdir"
# (df.write.mode("overwrite").parquet(tmp_dir))

# part = glob.glob(os.path.join(tmp_dir, "part-*.parquet"))[0]
# os.makedirs(os.path.dirname(output_parquet), exist_ok=True)
# shutil.copy(part, output_parquet)
# print(f"[OK] Wrote {output_parquet}")
...
# output_parquet nhận từ argv/env như bạn đã làm
tmp_dir = output_parquet + ".tmpdir"

# ghi parquet (thư mục)
df.write.mode("overwrite").parquet(tmp_dir)

# lấy 1 part-file và copy thành 1 file duy nhất
parts = sorted(glob.glob(os.path.join(tmp_dir, "part-*.parquet")))
if not parts:
    raise RuntimeError(f"No part files in {tmp_dir}")

os.makedirs(os.path.dirname(output_parquet), exist_ok=True)
shutil.copy(parts[0], output_parquet)

# log để dễ debug từ Kestra
print(f"###WROTE_PARQUET### {output_parquet}")
